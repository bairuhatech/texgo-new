...
const Features: React.FC = () => {
  ...
  const featureIcons = [
    Calculator, Package, Users, CreditCard, ShoppingCart, Monitor,
    BarChart3, Smartphone, ChefHat, Globe, Store, Database
  ];
  
  const featureColors = [
    "from-blue-500 to-blue-600", "from-green-500 to-green-600", "from-purple-500 to-purple-600",
    "from-yellow-500 to-yellow-600", "from-red-500 to-red-600", "from-indigo-500 to-indigo-600",
  ...
  const features = translations.features.items.map((item: any, index: number) => ({
  ...
  return (
    <section id="products" className="py-20 bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-900 relative overflow-hidden">
  ...
  {React.createElement(feature.icon, { className: "w-7 h-7 text-white" })}
                ...
                {/* Content */}
                <h3 className="text-lg font-bold text-deep-black dark:text-white mb-3 group-hover:text-primary-yellow transition-colors duration-300">
                  {feature.title}
                </h3>
